...
class SiteGenerator {
 ...
 private:
  ...
  void SubmitTranslate(const fs::path& fpath, const fs::path& new_root,
                       const fs::path& dir_path) {
    Execute(pool_, [this, dir_path, fpath, new_root]() {
      auto html_content = Translator().GmI2Html(dir_path, out_dir_);
      auto path =
          ReplaceRoots(in_dir_, out_dir_, dir_path).replace_extension("html");
      SubmitWrite(path, html_content);
    });
  }

  /*
   * Обход дерева каталога: файлы, которые нужно просто скопировать,
   * распределяются по батчам и отправляются на исполнение.
   */
  void Traverse() {
    CopyBatch batch;
