...
class Translator {
  /*
   * Логика обработки ссылок сложнее остальных тегов, поэтому вынесем обработку
   * в этот метод.
   */
  void LinkConnector(std::string line, const fs::path& new_root = "") {
    ...
    if (line.rfind("http", 0) == 0 ||
      ...
      // inner links. Считаем, что все пути относительные (в корне, который
      // передается первым аргументом) Также, пусть в исходном проекте все
      // ссылки на другие страницы в формате .gmi (т.е. нет ссылок на .html).
    ...
    result_.append("<p><a href=\"" + link + "\">" + name + "</a></p>");
  }
 ...
 public:
  std::string GmI2Html(const fs::path& fpath, const fs::path& new_root) {
    ...
    for (std::string line; getline(infile, line);) {
      ...
      if (tag_ == "#") {
        result_.append("<h1>" + line.substr(token_st) + "</h1>");
      } else if (tag_ == "##") {
        result_.append("<h2>" + line.substr(token_st) + "</h2>");
      } else if (tag_ == "###") {
        result_.append("<h3>" + line.substr(token_st) + "</h3>");
      } else if (tag_ == "*") {
        result_.append("<li>" + line.substr(token_st) + "</li>");
      } else if (tag_ == ">") {
        result_.append("<blockquote><p>" + line.substr(token_st) +
                       "</p></blockquote>");
      } else if (tag_ == "=>") {
