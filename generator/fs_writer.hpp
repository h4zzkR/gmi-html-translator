...
class FSWriter {
 public:
  /*
   * batch мувается в лямбду, а сама лямбда уже внутри Execute аллоцируется на
   * куче, управление памятью переходит к тредпулу, поэтому объект живет до тех
   * пор, пока не исполнится.
   */
  void Copy(CopyBatch batch) {
    Execute(thread_, [this, batch2run = std::move(batch)]() {
      for (const fs::directory_entry& dir_entry : batch2run.buckets) {
        auto new_path =
            ReplaceRoots(root_dir_, new_root_dir_, dir_entry.path());

        if (dir_entry.is_directory()) {
          fs::create_directory(new_path);
        } else {
          fs::copy_file(dir_entry, new_path,
  ...
  FSWriter(const fs::path& root_dir, const fs::path& new_root_dir)
      : root_dir_(root_dir), new_root_dir_(new_root_dir) {
  }
